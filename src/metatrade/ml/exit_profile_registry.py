"""Registry for trained exit profile selector models.

Provides save/load/promote semantics for exit profile models.

Each model is saved as:
    {registry_dir}/{symbol}_exitprofile_{version}.pkl

Active version pointer:
    {registry_dir}/{symbol}_exitprofile_active.txt
...
@dataclass
class ExitProfileModelSnapshot:
    """Metadata and trained classifier for one exit profile model.

    Attributes:
        version:       Version string, e.g. ``"exitprofile_v20240601_1430"``.
        model:         Trained sklearn-compatible classifier.
        profile_ids:   Class labels (profile_id strings) the model knows.
...
class ExitProfileModelRegistry:
    """File-based registry for exit profile selector models.
    ...
    def __init__(self, registry_dir: str | Path, symbol: str = "EURUSD") -> None:
        ...
        self._symbol = symbol
        self._snapshots: dict[str, ExitProfileModelSnapshot] = {}
    ...
    def promote(self, version: str) -> None:
        ...
        if version not in self._snapshots:
            raise KeyError(
                f"ExitProfileModelRegistry: version '{version}' not in registry"
            )
        self._active_version = version
        log.info(
            "exit_profile_model_promoted", version=version, symbol=self._symbol
        )

    def get_active(self) -> ExitProfileModelSnapshot | None:
    ...
    def save_to_disk(self, version: str | None = None) -> Path | None:
        ...
        if ver is None:
            log.warning("exit_profile_save_no_version")
            return None
        snap = self._snapshots.get(ver)
        if snap is None:
            log.warning("exit_profile_save_version_not_found", version=ver)
            return None

        self._dir.mkdir(parents=True, exist_ok=True)
        path = self._dir / f"{self._symbol}_exitprofile_{ver}.pkl"
        try:
            with open(path, "wb") as fh:
                pickle.dump(snap, fh, protocol=pickle.HIGHEST_PROTOCOL)
            # Update active pointer file
            if ver == self._active_version:
                (self._dir / f"{self._symbol}_exitprofile_active.txt").write_text(ver)
            log.info("exit_profile_model_saved", path=str(path), version=ver)
            return path
        except Exception as exc:
            log.error("exit_profile_model_save_failed", error=str(exc))
            return None

    def load_from_disk(
        self, version: str | None = None
    ) -> ExitProfileModelSnapshot | None:
        ...
        Returns:
            ExitProfileModelSnapshot, or None if not found.
        """
        if version is None:
            ptr = self._dir / f"{self._symbol}_exitprofile_active.txt"
            if ptr.exists():
                version = ptr.read_text().strip()
            if not version:
                return None

        path = self._dir / f"{self._symbol}_exitprofile_{version}.pkl"
        if not path.exists():
            return None

        try:
            with open(path, "rb") as fh:
                snap: ExitProfileModelSnapshot = pickle.load(fh)
            self._snapshots[snap.version] = snap
            log.info(
                "exit_profile_model_loaded",
            ...
            log.error(
                "exit_profile_model_load_failed", path=str(path), error=str(exc)
            )
            return None

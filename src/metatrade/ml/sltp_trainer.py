"""Walk-forward training pipeline for the SL/TP policy classifier.

Trains a multiclass classifier (one class per policy_id) on the dataset
produced by SlTpDatasetBuilder using a time-ordered walk-forward scheme
...
Walk-forward scheme
-------------------
The dataset rows are sorted by timestamp.  Each fold trains on the first
N rows and evaluates on the next K rows.  The final model is trained on
all data after folds are evaluated.
"""
...
from metatrade.ml.sltp_dataset_builder import DatasetRow
...
class SlTpFoldMetrics:
    ...
    n_test: int
    accuracy: float
    per_policy_accuracy: dict[str, float] = field(default_factory=dict)


@dataclass
class SlTpTrainingResult:
    """Full output of a training run."""

    model: Any                         # trained sklearn-compatible classifier
    policy_ids: list[str]              # class labels (policy_id strings)
    feature_names: tuple[str, ...]     # ordered feature columns
    folds: list[SlTpFoldMetrics]
    mean_accuracy: float
    n_total_samples: int
...
def _rows_to_arrays(
    ...
    Feature order is taken from SlTpFeatureBuilder.FEATURE_NAMES so that
    every training run and inference call uses the same column order.
    """
    if not rows:
        return np.zeros((0, len(SlTpFeatureBuilder.FEATURE_NAMES))), []

    feature_keys = list(SlTpFeatureBuilder.FEATURE_NAMES)
    X = np.array(
...
class SlTpTrainer:
    """Trains an SL/TP policy classifier with walk-forward validation.
    ...
    def train(self, rows: list[DatasetRow]) -> SlTpTrainingResult | None:
        """Train the classifier with walk-forward validation.

        Args:
            rows: Dataset rows from SlTpDatasetBuilder, sorted by time.

        Returns:
            SlTpTrainingResult with the final model, or None when there
            is insufficient data.
        """
        if len(rows) < self._min_train_rows:
            log.warning(
                "sltp_insufficient_training_data",
                n_rows=len(rows),
        ...
        n = len(rows_sorted)
        policy_ids = sorted(set(y_all))

        folds: list[SlTpFoldMetrics] = []
        # Use expanding window: fold i tests on the last (i+1) * fold_size rows
        ...
        for fold_idx in range(self._n_splits - 1, -1, -1):
            ...
            try:
                ...
                for pid in set(y_te):
                    ...
                    if any(mask):
                        per_policy[pid] = float(
                            np.mean(
                                np.array(preds)[mask] == np.array(y_te)[mask]
                            )
                        )

                folds.append(SlTpFoldMetrics(
                    fold=fold_idx,
                    n_train=train_end,
                ...
                log.warning("sltp_fold_failed", fold=fold_idx, error=str(exc))

        if not folds:
            log.warning("sltp_no_valid_folds", n_rows=n)
            return None
        ...
        try:
            ...
            log.error("sltp_final_train_failed", error=str(exc))
            return None

        mean_acc = float(np.mean([f.accuracy for f in folds]))
        log.info(
            "sltp_training_complete",
            n_samples=n,
            n_folds=len(folds),
        ...
        return SlTpTrainingResult(
            model=final_clf,
            policy_ids=policy_ids,
            feature_names=SlTpFeatureBuilder.FEATURE_NAMES,
            folds=folds,
